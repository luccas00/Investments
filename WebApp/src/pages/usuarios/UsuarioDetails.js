...
export default function UsuarioDetails() {
  ...
  const formatType = (t) => ({ ACAO: 'Ação', RENDA_FIXA: 'Renda Fixa', CRIPTO: 'Criptomoedas', FUNDO: 'Fundo de Investimento' , OUTROS: 'Outros' }[t] ?? t);

  const formatPrice = (v) => `${Number(v ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} R$`;
  ...
  return (
    <div>
      ...
      <h3>Investimentos</h3>
        <ul>
            {usuario?.investments?.map((inv) => (
                <li key={inv.id}>
                    {formatType(inv.type)}, {inv.symbol}, {inv.status}, {inv.quantity}, {formatPrice(inv.purchasePrice)}
                </li>
            ))}
        </ul>
